function p = kSpaceInverseMirrorFFT2D(f,setting)
...
%                'computation.interpolationMethodI' - inverse interpolation method
...
interpolationMethod = setting.computation.interpolationMethodI;
...
sf = c.^2 .* real(sign(w).*sqrt( (w ./ c).^2 - datakgrid.ky.^2)) ./ w;
sf(isnan(sf)) = 0;
sf(w == 0 & datakgrid.ky == 0) = c; % handle the singularity
