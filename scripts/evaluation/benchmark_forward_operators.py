...
def main() -> None:
    ...
    for condition in conditions(args.condition):
        ...
        if args.include_kwave:
            # k-Wave is a CPU NumPy implementation; measure the same number of samples.
            def kwave_pipeline(setting=setting, p0_np=p0_np):
                return [kwave_forward_2d(item, setting) for item in p0_np]

            durations = time_callable(
                kwave_pipeline, args.warmup, args.repeats, torch.device("cpu")
            )
            rows.append(
                summary("k-Wave", durations, count, device="cpu", parameters=0, checkpoint_bytes=0)
            )
